...
for idata=1:length(dataname)  
    dataf = strcat(loaddata,dataname(idata),'/',lower(dataname(idata)),'_cites.mtx'); %Just the datafile name
    X1 = importdata(cell2mat(dataf(1)), delimeter, headersInFile);
    X1 = X1.data;
    dataf = strcat(loaddata,dataname(idata),'/',lower(dataname(idata)),'_content.mtx'); %Just the datafile name
    X2 = importdata(cell2mat(dataf(1)), delimeter, headersInFile);
    X2 = X2.data;
    dataf = strcat(loaddata,dataname(idata),'/',lower(dataname(idata)),'_act.txt'); %Just the datafile name
